...
function App() {
  ...
  const resetGame = () => {
    ...
    setSelectedSquare(null); setLegalMoves([]); setGameOverMsg(""); setIsThinking(false);
    ...
    if (mode === "AI" && humanColor === "b") {
      ...
      setTimeout(() => {
        ...
        if (aiMove) { newGame.move(aiMove); playSfx(aiMove.captured ? 'capture' : 'move'); }
  ...
  const doResetGameAndStart = () => {
    ...
    setGameOverMsg(""); setSelectedSquare(null); setLegalMoves([]); playSfx('start');
  };

  const handleOnlineData = useCallback((data) => {
    if (data.type === 'MOVE') {
      const move = gameRef.current.move(data.move);
      if (move) playSfx(move.captured ? 'capture' : 'move');
  ...
  const connectMQTT = (onReady) => {
    ...
    const cli = window.mqtt.connect('wss://public:public@public.cloud.shiftr.io', { clientId: 'vnc_' + netIdRef.current });
      ...
      if (move) {
        ...
        if (mode === "AI" && !game.game_over()) {
          ...
          setTimeout(() => {
            ...
            if (aiMove) { game.move(aiMove); updateBoardInfo(); playSfx(aiMove.captured ? 'capture' : 'move'); }
            setIsThinking(false);
          }, 50);
        }
      } else {
